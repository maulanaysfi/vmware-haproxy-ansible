...
main() {
  ...
  while true; do
    read -ep "[?] Confirm configuration? (y=Yes|n=No|r=Repeat): " choice
    case "$choice" in
      y|Y ) 
        break 
        ;;
  ...
  while true; do
    read -ep "[?] Proceed to Ansible Installation? (y=Yes|n=No): " choice
    case "$choice" in
      y|Y ) 
        break 
        ;;
...
generate_yml() {
  ...
  echo "$MANAGEMENT_IPV4" > inventory
  echo "[!] Successfully wrote Management IPv4 to inventory."
}
